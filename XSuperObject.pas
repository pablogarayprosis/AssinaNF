...
type
   ...
   public
      ...
      class procedure ReadMember<T: Class; Typ>(Member: Typ;
        TypeKind: TTypeKind; MemberValue: TValue; IJsonData: IBaseJSON<T, Typ>);
      ...
      class procedure WriteMembers(Data: Pointer; aType: TRttiType;
        IJsonData: ISuperObject);
      class procedure WriteMember<T: Class; Typ>(Data: Pointer; Member: Typ;
        TypeKind: TTypeKind; MemberValue: TRttiMember;
        IJsonData: IBaseJSON<T, Typ>);
      class procedure WriteSet(Data: Pointer; Member: TRttiMember;
        IJsonData: ISuperArray);
      class procedure SetValue<Typ>(Data: Pointer; Member: TRttiMember;
        MIdx: Typ; Val: TValue);
      class function GetValue<Typ>(Data: Pointer; Member: TRttiMember;
        MIdx: Typ): TValue;
      class function GetMemberTypeInfo(Member: TRttiMember;
        const GetArray: Boolean = true): PTypeInfo; inline;
      class function GetMemberType(Member: TRttiMember;
      ...
      class function CheckObject(Data: Pointer; Member: TRttiMember;
   ...
   begin
      ...
      if TJSONArray(FJSONObj).Size - 1 < Index then
         while TJSONArray(FJSONObj).Size - 1 < Index do
            TJSONArray(FJSONObj).AddElement(DefaultValueClass<T>(Value)
              as TJSONValue);
   end;
...
begin
   ...
   if Temp = Nil then
      ...
      Result := varString
   else if Temp.ClassType = TJSONNumber then
   begin
...
begin
   case VarType(Data) of
      ...
      varInt64:
         FJSONObj.AddPair(V, TJSONNumber.Create(Int64(Data)));
   end;
end;
...
begin
   for Prop in aType.GetProperties do
      ReadMember<TJSONObject, String>(Prop.Name, Prop.PropertyType.TypeKind,
        Prop.GetValue(Data), IJsonData);
   for Field in aType.GetFields do
      ReadMember<TJSONObject, String>(Field.Name, Field.FieldType.TypeKind,
        Field.GetValue(Data), IJsonData);
end;
...
class function TSerializeParse.CheckObject(Data: Pointer; Member: TRttiMember;
  var Obj: TObject): Boolean;
begin
   Obj := Nil;
   ...
   begin
      ...
      if (Obj = Nil) then
         if (ObjectConstructorParamCount(TRttiProperty(Member)
           .PropertyType.AsInstance.MetaclassType) <> 0) then
            exit(False)
         else
         begin
            Obj := ObjectConstructor(TRttiProperty(Member)
   ...
   begin
      ...
      if (Obj = Nil) then
         if (ObjectConstructorParamCount(TRttiField(Member)
           .FieldType.AsInstance.MetaclassType) <> 0) then
            exit(False)
         else
         begin
            Obj := ObjectConstructor(TRttiProperty(Member)
...
class function TSerializeParse.GetMemberType(Member: TRttiMember;
...
class function TSerializeParse.GetMemberTypeInfo(Member: TRttiMember;
  const GetArray: Boolean): PTypeInfo;
begin
   Result := GetMemberType(Member, GetArray).Handle
end;

class function TSerializeParse.GetValue<Typ>(Data: Pointer; Member: TRttiMember;
  MIdx: Typ): TValue;
begin
   if (TypeInfo(Typ) = TypeInfo(Integer)) and
     (GetMemberTypeInfo(Member, False).Kind = tkDynArray) then
   ...
   case TypeKind of
      ...
      tkVariant:
         if TypeInfo(Typ) = TypeInfo(String) then
            ReadVariantOfObject(MemberValue.AsVariant, TValue.From<Typ>(Member)
              .AsString, ISuperObject(IJsonData));
      ...
      tkArray, tkDynArray:
         with MemberValue do
            ...
            begin
               ...
               ReadMember<TJSONArray, Integer>(I, SubVal.Kind, SubVal,
                 IJsonData.A[Member]);
            end;
...
class procedure TSerializeParse.SetValue<Typ>(Data: Pointer;
  Member: TRttiMember; MIdx: Typ; Val: TValue);
begin
   if (TypeInfo(Typ) = TypeInfo(Integer)) and
     (GetMemberTypeInfo(Member, False).Kind = tkDynArray) then
      GetValue<String>(GetArrayRawData(Member), Member, '')
        .SetArrayElement(TValue.From<Typ>(MIdx).AsInteger, Val)
...
begin
   ...
   case TypeKind of
      ...
      tkArray:
         raise Exception.Create('There is no support for static array.');
         ...
         begin
            ...
            for I := 0 to J - 1 do
               WriteMember<TJSONArray, Integer>
                 ...
                 GetMemberType(MemberValue).TypeKind, MemberValue,
                 IJsonData.A[Member]);
...
begin
   for Prop in aType.GetProperties do
      if Prop.PropertyType <> Nil then
         WriteMember<TJSONObject, String>(Data, Prop.Name,
           Prop.PropertyType.TypeKind, TSuperProperty(Prop), IJsonData);
   for Field in aType.GetFields do
      if Field.FieldType <> Nil then
         WriteMember<TJSONObject, String>(Data, Field.Name,
           Field.FieldType.TypeKind, TSuperField(Field), IJsonData);
end;
